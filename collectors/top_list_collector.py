"""
龙虎榜采集模块 - collectors/top_list_collector.py
采集龙虎榜每日上榜股票数据
...
# 输出文件
OUTPUT_FILE = 'data/top_list.csv'


class TopListCollector:
    """龙虎榜采集器（仅Tushare）"""
    ...
    def _get_progress_file(self):
        """获取进度文件路径"""
        return os.path.join(PROGRESS_DIR, 'top_list_progress.json')
    ...
    def collect(self, start_date, end_date, resume=True):
        ...
        for idx, trade_date in enumerate(pending_dates):
            ...
            try:
                ...
                logger.info(
                    f"top_list day={trade_date} rows={0 if df is None else len(df)} elapsed={elapsed:.2f}s"
                )
                print(f"   ⏱️ {elapsed:.2f}s, 累计 {total_rows} 条")
                
            except Exception as e:
                print(f"❌ {str(e)[:40]}")
                logger.error(f"top_list day={trade_date} failed: {e}")
