# -*- coding: utf-8 -*-
"""ETF模型训练器 - 迁移自 scripts/train_etf.py"""
...
class ETFTrainer:
    ...
    def train_model(self, period_days=5):
        ...
        can_calibrate = len(y_val_arr) >= 160 and len(np.unique(y_val_arr)) >= 2
        if can_calibrate:
            split = int(len(y_val_arr) * 0.5)
            split = max(80, min(split, len(y_val_arr) - 80))
            X_cal = X_val_scaled[:split]
            y_cal = y_val_arr[:split]
            X_eval = X_val_scaled[split:]
            y_eval = y_val_arr[split:]
        ...
        raw_eval_proba = self._predict_positive_proba(model, X_eval)
        raw_cal_proba = self._predict_positive_proba(model, X_cal) if can_calibrate else None

        method_candidates = ['none', 'platt', 'isotonic'] if can_calibrate else ['none']
        candidate_results = []
        for method in method_candidates:
            calibrator = self._fit_probability_calibrator(method, y_cal, raw_cal_proba) if method != 'none' else None
            ...
            # 先按 gate 约束选择阈值；若无通过候选，再退化为 F1 最优阈值
            threshold_candidates = np.arange(0.30, 0.70 + 1e-9, 0.01)
            passed_threshold_candidates = []
            ...
            for threshold in threshold_candidates:
                ...
                all_threshold_candidates.append(row)
                if passed_t:
                    passed_threshold_candidates.append(row)

            if passed_threshold_candidates:
                best_t = max(
                    passed_threshold_candidates,
                    key=lambda item: (item['validation_f1'], item['validation_accuracy'], item['validation_precision'])
                )
            else:
                best_t = max(
                    all_threshold_candidates,
            ...
            best_threshold = round(float(best_t['threshold']), 2)
            best_f1 = float(best_t['validation_f1'])
            eval_acc = float(best_t['validation_accuracy'])
            eval_precision = float(best_t['validation_precision'])
            eval_recall = float(best_t['validation_recall'])
            gate_name = best_t['validation_gate']
            gate_reason = best_t['validation_reason']
            passed = bool(best_t['validation_passed'])
            ...
            candidate_results.append({
                ...
                'calibration_samples': int(len(y_cal)) if method != 'none' and y_cal is not None else 0,
                'validation_accuracy': eval_acc,
                'validation_f1': best_f1,
                'validation_precision': eval_precision,
                'validation_recall': eval_recall,
        ...
        val_acc = best_candidate['validation_accuracy']
        best_f1 = best_candidate['validation_f1']
        val_precision = best_candidate['validation_precision']
        val_recall = best_candidate['validation_recall']
        best_threshold = best_candidate['decision_threshold']
        val_auc = best_candidate['validation_auc']
        val_brier = best_candidate['validation_brier']
        gate_name = best_candidate['validation_gate']
        gate_reason = best_candidate['validation_reason']
        gate_passed = best_candidate['validation_passed']
        cal_method = best_candidate['calibration_method']
        calibrator_obj = best_candidate['calibrator']
        calibration_samples = best_candidate['calibration_samples']

        print(f'  📊 训练准确率: {train_acc:.2%}')
        print(f'  📊 验证准确率: {val_acc:.2%}')
        print(f'  📊 验证F1(最优阈值): {best_f1:.2%} (threshold={best_threshold:.2f}, precision={val_precision:.2%}, recall={val_recall:.2%})')
        print(f'  📊 概率校准: {cal_method} (samples={calibration_samples}) | gate={gate_name} | passed={gate_passed}')
        ...
        if horizon == 20 and gate_passed:
            with open(os.path.join(self.models_dir, 'etf_model.pkl'), 'wb') as f:
                pickle.dump(model_data, f)
        elif horizon == 20 and not gate_passed:
            print('  ⚠️ 20日ETF模型未通过validation gate，保留现有 etf_model.pkl 不覆盖')
