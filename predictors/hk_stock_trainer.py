# -*- coding: utf-8 -*-
"""港股模型训练器 - 迁移自 scripts/train_hk_stock.py"""
...
class HKStockTrainer:
    ...
    def train_model(self, period_days, model_key):
        ...
        if period_days <= 5:
            if HAS_XGB:
                ...
                model = GradientBoostingClassifier(
                    n_estimators=80, learning_rate=0.04, max_depth=2, random_state=42
                )
        ...
        for method in method_candidates:
            ...
            for threshold in np.arange(0.30, 0.66 + 1e-9, 0.01):
        ...
        # 保存模型
        model_file = os.path.join(self.models_dir, f'hk_stock_{model_key}_model.pkl')
        model_data = {
            'model': model,
            'scaler': scaler,
